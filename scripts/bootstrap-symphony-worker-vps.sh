...
WORKSPACE_ROOT="${WORKSPACE_ROOT:-$WORKER_HOME/code/symphony-workspaces}"
...
log() {
  printf '[bootstrap-symphony-worker] %s\n' "$*"
}
...
clone_or_update_repo() {
  ...
  if [[ -d "${target_dir}/.git" ]]; then
    log "Updating ${label} repository at ${target_dir}."
    git config --global --add safe.directory "${target_dir}"
    git -C "${target_dir}" fetch origin "${branch}" --depth=1
    git -C "${target_dir}" checkout "${branch}"
    git -C "${target_dir}" pull --ff-only origin "${branch}"
  else
    log "Cloning ${label} repository to ${target_dir}."
    git clone --depth=1 --branch "${branch}" "${repo_url}" "${target_dir}"
  fi
}

prepare_repositories() {
  clone_or_update_repo "${WORKFLOW_REPO_URL}" "${WORKFLOW_REPO_BRANCH}" "${WORKFLOW_REPO_DIR}" "workflow"
  clone_or_update_repo "${SYMPHONY_REPO_URL}" "${SYMPHONY_REPO_BRANCH}" "${SYMPHONY_REPO_DIR}" "Symphony"

  if [[ ! -f "${WORKFLOW_REPO_DIR}/WORKFLOW.md" ]]; then
    echo "WORKFLOW.md not found in ${WORKFLOW_REPO_DIR}." >&2
    exit 1
  fi

  chown -R "${WORKER_USER}:${WORKER_GROUP}" "${WORKFLOW_REPO_DIR}" "${SYMPHONY_REPO_DIR}"
}
...
write_env_file() {
  log "Writing /etc/symphony-worker.env."
  cat >/etc/symphony-worker.env <<EOF
HOME=${WORKER_HOME}
PATH=/usr/local/bin:/usr/bin:/bin:${WORKER_HOME}/.local/bin
LINEAR_API_KEY=${LINEAR_API_KEY}
PLAYWRIGHT_BROWSERS_PATH=${PLAYWRIGHT_BROWSERS_PATH}
SYMPHONY_WORKFLOW_FILE=${WORKFLOW_REPO_DIR}/WORKFLOW.md
SYMPHONY_LOGS_ROOT=${SYMPHONY_LOGS_ROOT}
SYMPHONY_WORKSPACE_ROOT=${WORKSPACE_ROOT}
EOF

  if [[ -n "${SYMPHONY_DASHBOARD_PORT}" ]]; then
    echo "SYMPHONY_DASHBOARD_PORT=${SYMPHONY_DASHBOARD_PORT}" >> /etc/symphony-worker.env
  fi

  chmod 600 /etc/symphony-worker.env
}

write_start_script() {
  log "Writing /usr/local/bin/symphony-worker-start."
  cat >/usr/local/bin/symphony-worker-start <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
  ...
  chmod 755 /usr/local/bin/symphony-worker-start
}
...
[Unit]
Description=Symphony worker for ALFAB
...
EnvironmentFile=/etc/symphony-worker.env
ExecStart=/usr/local/bin/symphony-worker-start
Restart=always
RestartSec=10
...
enable_service() {
  log "Enabling and starting symphony-worker.service."
  systemctl daemon-reload
  systemctl enable --now symphony-worker.service
}

print_summary() {
  cat <<EOF

Symphony worker bootstrap complete.
- Service: symphony-worker.service
- Workflow repo: ${WORKFLOW_REPO_DIR}
- Symphony repo: ${SYMPHONY_REPO_DIR}
- Workflow file: ${WORKFLOW_REPO_DIR}/WORKFLOW.md
