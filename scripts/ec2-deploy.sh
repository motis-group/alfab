...
server {
    ...
    # An order arrives as a scan: nginx defaults to 1 MB, and the import route accepts 30 MB.
    client_max_body_size 30m;

    location / {
        proxy_http_version 1.1;
        # Reading a drawing takes longer than nginx's 60 s default. The route allows 300 s.
