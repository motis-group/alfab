...
export default function ProductsPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [categoryRes, productRes] = await Promise.all([
        db.from(TABLE_PRODUCT_CATEGORIES).select('*').order('name', { ascending: true }),
  ...
  async function saveCategory() {
    ...
    try {
      ...
      if (categoryForm.id) {
        const { error: updateError } = await db.from(TABLE_PRODUCT_CATEGORIES).update(payload).eq('id', categoryForm.id);
        if (updateError) throw updateError;
      } else {
        const { error: insertError } = await db.from(TABLE_PRODUCT_CATEGORIES).insert(payload);
  ...
  async function saveProduct() {
    ...
    try {
      ...
      if (productForm.id) {
        const { error: updateError } = await db.from(TABLE_PRODUCTS).update(payload).eq('id', productForm.id);
        if (updateError) throw updateError;
      } else {
  ...
  async function toggleProductActive(product: Product) {
    ...
    try {
      const db = createClient();
      const { error: updateError } = await db.from(TABLE_PRODUCTS).update({ is_active: product.is_active === false }).eq('id', product.id);
      if (updateError) throw updateError;
  ...
  return (
    <AppFrame
      ...
      heading="PRODUCT & CONFIGURATION CATALOG"
      badge={`${products.filter((product) => product.is_active !== false).length} ACTIVE`}
      showThemeControls
      actionItems={[
        {
          hotkey: '⌘+S',
    ...
    >
          ...
          <Table>
            ...
            {products.map((product) => (
              <TableRow key={product.id}>
                ...
                <TableColumn>
                  <span className={activeStatusClassName(product.is_active)}>{product.is_active === false ? 'INACTIVE' : 'ACTIVE'}</span>
                </TableColumn>
                <TableColumn>
                  <RowSpaceBetween>
        ...
        <Card title="SUMMARY">
          <RowSpaceBetween>
            ...
            <Text>
              <span className="status-success">{products.filter((product) => product.is_active !== false).length}</span>
            </Text>
          </RowSpaceBetween>
          <RowSpaceBetween>
