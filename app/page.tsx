...
import { GlassType, EdgeworkType, GlassSpecification, CostBreakdown, calculateCost, getAvailableThicknesses, getAvailableGlassTypes, SavedCalculation } from '@utils/calculations';
...
export default function CostingDashboard() {
  ...
  // Load saved calculations from PostgreSQL on mount
  useEffect(() => {
    const fetchQuotes = async () => {
      try {
        const db = createClient();
        const { data, error } = await db.from(TABLE_NAME).select('*').order('date', { ascending: false });
  ...
  const handleSaveCalculation = async () => {
    ...
    try {
      ...
      if (editingQuoteId) {
        ...
        // Create new quote - remove id property to let PostgreSQL generate it
        ...
        const { data, error } = await db.from(TABLE_NAME).insert(newCalculation).select();
  ...
  return (
    <AppFrame
      ...
      navLabel="GLASS COSTING"
      navRight={<ActionButton>SIGN OUT</ActionButton>}
      heading="GLASS COSTING DASHBOARD"
      badge={`${savedCalculations.length} QUOTES`}
      showThemeControls
    >
