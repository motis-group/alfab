...
const glassColors: GlassColor[] = ['Clear', 'Green', 'Grey', 'Dark Grey', 'Super Grey'];
...
const ceramicFullCoverPrices: Record<GlassType, Partial<Record<number, number>>> = {
  CLEAR: {
    5: 179.91,
    6: 195.73,
  },
...
const calculateGlassPrice = ({ widthMm, heightMm, thicknessMm, glassType, treatment = 'TOUGHENED', edgeworkType = 'FLAT GRIND - STRAIGHT', isCeramicFullCover = false, shapeType = 'RECTANGLE', customShape = '', hasHoles = false, numHoles = 0, ceramicBanding = false }: GlassSpecification): CostBreakdown => {
  ...
  if (!basePrice) {
    throw new Error(`Invalid combination of glass_type (${glassType}) and thickness (${thicknessMm}mm)`);
  }
...
const calculateCost = (spec: GlassSpecification): CostBreakdown => {
  ...
  try {
    const costs = calculateGlassPrice({
      ...
      treatment: 'TOUGHENED',
      edgeworkType: spec.edgeworkType,
      isCeramicFullCover: false,
      shapeType: spec.shapeType,
      hasHoles: spec.hasHoles,
      numHoles: spec.numHoles,
      ceramicBanding: spec.ceramicBanding,
    });
