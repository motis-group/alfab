...
export default function CustomersPage() {
  ...
  async function saveCustomer() {
    ...
    try {
      ...
      if (customerForm.id) {
        const { error: updateError } = await supabase.from(TABLE_CUSTOMERS).update(payload).eq('id', customerForm.id);
  ...
  async function toggleCustomerActive(customer: Customer) {
    ...
    try {
      const supabase = createClient();
      const { error: updateError } = await supabase.from(TABLE_CUSTOMERS).update({ is_active: customer.is_active === false }).eq('id', customer.id);
      if (updateError) throw updateError;
  ...
  async function saveCustomerConfig() {
    ...
    try {
      ...
      if (configForm.id) {
        const { error: updateError } = await supabase.from(TABLE_CUSTOMER_PRODUCTS).update(payload).eq('id', configForm.id);
        if (updateError) throw updateError;
      } else {
        const { error: insertError } = await supabase.from(TABLE_CUSTOMER_PRODUCTS).insert(payload);
        if (insertError) throw insertError;
      }
  ...
  async function deleteCustomerConfig(configId: string) {
    ...
    try {
      const supabase = createClient();
      const { error: deleteError } = await supabase.from(TABLE_CUSTOMER_PRODUCTS).delete().eq('id', configId);
  ...
  return (
    <DefaultLayout previewPixelSRC="/pixel.gif">
      ...
      <Navigation
        ...
        left={
          <DropdownMenuTrigger items={navigationItems}>
            <ActionButton>CUSTOMERS</ActionButton>
          </DropdownMenuTrigger>
        }
      ...
      <Grid>
        <Row>
          CUSTOMER MANAGEMENT <Badge>{customers.filter((customer) => customer.is_active !== false).length} ACTIVE</Badge>
        </Row>
            ...
            <Table>
              ...
              {customers.map((customer) => (
                <TableRow key={customer.id}>
                  ...
                  <TableColumn>{customer.is_active === false ? 'INACTIVE' : 'ACTIVE'}</TableColumn>
                  <TableColumn>
                    <RowSpaceBetween>
                      <ActionButton
                        onClick={() => {
        ...
        <Card title="SUMMARY">
          <RowSpaceBetween>
            ...
            <Text>{customers.filter((customer) => customer.is_active !== false).length}</Text>
          </RowSpaceBetween>
          <RowSpaceBetween>
